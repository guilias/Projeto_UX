# Projeto de Experiência de Usuário (WIP)

> Documentação e projeto em desenvolvimento, utilizando como base o repositório indicado pelo orientador da disciplina -  https://github.com/FagnerPimentel-Academic/template-ux/tree/main.

Projeto apresentado ao Centro Universitário [FEI](https://portal.fei.edu.br/), como parte dos requisitos necessários para aprovação na disciplina de Experiência do Usuário (CCP310) do curso de Ciencia da Computação, orientado pelo Prof. Dr. [Fagner de Assis Moura Pimentel](http://lattes.cnpq.br/6747210702910392).

Este projeto é desenvolvido pelos seguintes alunos:

- Gabriel Chicole Frederichi
- Giovana de Carvalho Martins
- Guilherme Elias Barrence Ribeiro

## Sobre o Problema

- Apresente uma breve descrição.
- Apresente o objetivo.
- Apresente o usuário final.
- Apresente os principais benefícios para o usuários.
- Apresente as funcionalidades.
- Apresente as tecnologias e ferramentas computacionais utilizadas.
- Apresente o contexto de uso.
## Desenvolvimento

### Descoberta

- [Análise de Concorrência](docs/01_Concorrencia.md)
- [Pesquisa e Coleta de Dados com Usuários](#)
- [Perfil do Usuário](#)
- [Personas](#)
- [Cenário de Análise/Problema](#)

### Definição

- [Análise de Tarefas](#)
- [Requisitos de UX e Metas de Usabilidade](#)
- [Arquitetura de Informação e Fluxo do Usuário](#)

### Prototipação

- [Prototipação em Papel](#)
- [Prototipação de Alta Fidelidade (FIGMA)](#)

### Avaliação

- [Planejamento da Avaliação](#)
- [Avaliação de Usabilidade através de Inspeção Heurística](#)
- [Teste de Usabilidade com Observação do Usuário](#)
